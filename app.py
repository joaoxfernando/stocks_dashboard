...
def build_sidebar():
    ...
    tickers_list = pd.read_csv('tickers_ibra.csv', index_col=0)
    tickers = st.multiselect(label='Selecione a(s) empresa(s)',
...
def build_main(tickers, prices):
    ...
    my_grid = grid(4,4,4,4,4,4, vertical_align='top')
    ...
    for t in prices.columns:
        ...
        if t == 'portfolio':
            colA.image('images/chart.png')
        elif t == 'IBOV':
            colA.image('images/stocks.png', width=85)
        else:
...
st.title('Dashboard para Ações Brasileiras')
